...
# -----------------------------------------------------
# ENTERPRISE-GRADE SLACK MESSAGE FORMAT (FINAL & CLEAN)
# -----------------------------------------------------
def build_slack_message(
    ...
    with safe formatting and clean Slack rendering.
    """
    ...
    return (
        f"🚨 *STRATEGIC INTELLIGENCE ALERT — {company} ({ticker})*\n\n"
        f"*FINAL TRADING SIGNAL:* *{signal}*\n\n"
        f"*Executive Summary*\n"
        f"• Current Price: ${last_price:,.2f}\n"
        f"• Forecasted Price (7-Day Avg): {forecast_price_str}\n"
        f"• Price Change (7-Day): {pct_change_str}\n"
        f"• Aggregate Sentiment: {sentiment_score:+.2f} ({sentiment_label})\n"
        f"• Forecasting Model: Prophet\n\n"
        f"*Strategy Insight*\n"
        f"{llm_summary.strip()}\n\n"
        f"_Generated by Real-Time Market Intelligence System_"
    )
