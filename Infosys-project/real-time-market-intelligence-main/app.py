...
def build_slack_message(
    ...
    """
    Builds a clean, executive-readable Slack alert.
    """
    ...
    return (
        f"🚨 *STRATEGIC INTELLIGENCE ALERT*\n\n"
        f"*Company:* {company} ({ticker})\n"
        f"*Final Trading Signal:* *{signal}*\n\n"
        f"*Executive Summary*\n"
        f"• Current Price: ${last_price:,.2f}\n"
        f"• 7-Day Forecast (Avg): {forecast_price_str}\n"
        f"• Expected Price Change: {pct_change_str}\n"
        f"• Market Sentiment: {sentiment_score:+.2f} ({sentiment_label})\n"
        f"• Forecast Model: Prophet\n\n"
        f"*Strategy Insight*\n"
        f"{llm_summary.strip()}\n\n"
